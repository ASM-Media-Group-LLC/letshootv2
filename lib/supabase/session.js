...
export async function getUserProfile() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, email, full_name, role, onboarding_status, legal_first_name, legal_last_name, date_of_birth, country, phone, stage_name, payment_status, lora_status, id_rejection_reason, capabilities, handle, avatar_url')
    .eq('id', user.id)
    .single();
  return { user, profile };
}
