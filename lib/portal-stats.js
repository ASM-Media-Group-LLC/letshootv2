...
// Initials for an author avatar (up to 2 words).
export function initials(name) {
  // Solo cuenta palabras que empiezan con letra/número — así "Alvaro (Owner)"
  // da "A" y no "A(" (ignora paréntesis, símbolos, roles entre signos).
  const parts = String(name || '').trim().split(/\s+/).filter((p) => /^[\p{L}\p{N}]/u.test(p));
