...
const TEMPLATES: Record<string, (name: string, extra: string, lang: string, url: string) => { subject: string; html: string }> = {
  ...
  // Invitación a una PROPUESTA privada de fotos. `url` = link público de la propuesta
  // (https://www.letshoot.ai/p/<link>); `extra` = nombre de la modelo de la selección.
  proposal: (name, extra, lang, url) => lang === 'es'
    ? { subject: 'Tu propuesta privada te está esperando', html: layout({ accent: BRAND, eyebrow: 'Selección privada', title: `${name}, te preparamos una propuesta`, body: `Echa un vistazo a la selección de looks que armamos${extra ? ` con ${extra}` : ''} y marca los que más te gusten. Es privada, toma un par de minutos y tu opinión llega directo a nuestro equipo.`, cta: 'Ver mi propuesta', url: url || `${APP}/propuestas`, pre: 'Tu propuesta privada de LetShoot te está esperando.' }) }
    : { subject: 'Your private proposal is waiting', html: layout({ accent: BRAND, eyebrow: 'Private selection', title: `${name}, we made you a proposal`, body: `Take a look at the selection of looks we put together${extra ? ` with ${extra}` : ''} and mark the ones you like best. It's private, takes a couple of minutes, and your feedback goes straight to our team.`, cta: 'See my proposal', url: url || `${APP}/propuestas`, pre: 'Your private LetShoot proposal is waiting.' }) },
  ...
  // Agency-facing: one of the agency's models was approved/rejected. `extra` = model name.
  model_approved: (name, extra, lang) => lang === 'es'
    ? { subject: 'Una de tus modelos fue verificada', html: layout({ accent: BRAND, eyebrow: 'Modelo verificada', title: `${extra || 'Tu modelo'} pasó la verificación`, body: `Buenas noticias, ${name}: la identidad de ${extra || 'tu modelo'} fue aprobada. Ya puede activar su plan y empezar a producir contenido.`, cta: 'Abrir mi panel', url: `${APP}/agencia`, pre: `${extra || 'Tu modelo'} fue verificada.` }) }
    : { subject: 'One of your models was verified', html: layout({ accent: BRAND, eyebrow: 'Model verified', title: `${extra || 'Your model'} passed verification`, body: `Good news, ${name}: ${extra || 'your model'}'s identity was approved. She can now activate her plan and start producing content.`, cta: 'Open my panel', url: `${APP}/agencia`, pre: `${extra || 'Your model'} was verified.` }) },
  model_rejected: (name, extra, lang) => lang === 'es'
    ? { subject: 'La verificación de tu modelo necesita un ajuste', html: layout({ accent: BRAND, eyebrow: 'Acción necesaria', title: `${extra || 'Tu modelo'} debe reenviar su ID`, body: `Hola ${name}: la verificación de identidad de ${extra || 'tu modelo'} no pasó esta vez. Le pedimos volver a subir su documento; te avisamos en cuanto quede aprobada.`, cta: 'Abrir mi panel', url: `${APP}/agencia`, pre: `${extra || 'Tu modelo'} debe reenviar su documento.` }) }
    : { subject: "Your model's verification needs a fix", html: layout({ accent: BRAND, eyebrow: 'Action needed', title: `${extra || 'Your model'} must re-upload her ID`, body: `Hi ${name}: ${extra || 'your model'}'s identity verification didn't pass this time. We asked her to re-upload her document; we'll let you know once it's approved.`, cta: 'Open my panel', url: `${APP}/agencia`, pre: `${extra || 'Your model'} must re-upload her document.` }) },
};
